...
int main(int argc, char *argv[])
{
	...
	else
	{
		...
		int selected_jobs = -1; //количество заданий для обработки. -1, если мы хотим прочитать все вакансии

		if (argc == 4) //если аргумент не передан, прочтите все задания
		{
			selected_jobs = atoi(argv[3]);
		}

		if (argv[1][1] == 'b') //Импортировать Orlib
		{
			jobProcessor->importOrlib(file_name, selected_jobs);
		}
		else if (argv[1][1] == 't') //Импортировать Taillard
		{
			jobProcessor->importTaillard(file_name, selected_jobs);
		}
		jobProcessor->printData(); //распечатываем наши входные данные

		//решить задачу цеха, используя метаэвристику моделирования отжига
		...
		char buffer[] = "../results.txt";	//сохранение результатов решения данного экземпляра в файле results.txt.
		FILE *results;
